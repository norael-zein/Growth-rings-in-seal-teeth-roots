...
CNN_WEIGHTS_PATH = "effnet_prod_model.pth"
CNN_DEVICE = "cpu"
...
_MEAN = torch.tensor([0.4533, 0.4691, 0.4300]).view(1, 3, 1, 1)
_STD  = torch.tensor([0.1011, 0.0967, 0.1070]).view(1, 3, 1, 1)

def preprocess_roi_for_bettercnn(roi_rgb: np.ndarray, device="cpu") -> torch.Tensor:
    if roi_rgb is None:
        raise ValueError("roi_rgb is None")

    roi_rgb = cv.resize(roi_rgb, (400, 200), interpolation=cv.INTER_AREA)
    x = torch.from_numpy(roi_rgb).permute(2, 0, 1).float().div_(255.0)
    x = x.unsqueeze(0)
    x = (x - _MEAN) / _STD
