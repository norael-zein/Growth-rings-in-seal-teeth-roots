...
def fig_to_rgb(plt_fig) -> np.ndarray:
    buf = io.BytesIO()
    plt_fig.savefig(buf, format="png", bbox_inches="tight", pad_inches=0.50, dpi=120)
    buf.seek(0)
...
def build_demo():
    ...
    with gr.Blocks(
        ...
        def change_index(i, imgs, pstore):
            if not imgs:
                return None, None, None, None, "", None, 0, "<span class='accent'>⚠️</span> <span class='muted'>No images.</span>"
        ...
        def on_click_orig_line(evt: gr.SelectData, curr_bgr, i, pstore):
            if curr_bgr is None:
                return None, None, None, pstore, "", "<span class='accent'>⚠️</span> <span class='muted'>Please upload images first.</span>"
            ...
            if roi_rgb is None:
                ...
                page["cnn_age"] = None
                pstore[int(i)] = page
                return None, None, None, pstore, "", "<span class='accent'>⚠️</span> <span class='muted'>Segment too short. Please re-click two points.</span>"
        ...
        def cnn_predict_current(i, pstore, progress=gr.Progress()):
            ...
            if page.get("roi_preview", None) is None:
                return pstore, "", "<span class='accent'>⚠️</span> <span class='muted'>No ROI preview. Please select ROI first.</span>"

            progress(0.10, desc="Loading CNN model...")
            try:
                model, device = load_cnn_model_cached(CNN_WEIGHTS_PATH, CNN_DEVICE)
            except Exception as e:
                return pstore, "", f"<span class='accent'>❌ CNN load failed:</span> <span class='muted'>{str(e)}</span>"
        ...
        def export_pdf_current(i, pstore):
            ...
            if page["orig_with_line"] is None or page["roi_preview"] is None or page["profile_img"] is None:
                return None, "<span class='accent'>⚠️</span> <span class='muted'>This page has no ROI results.</span>"
            ...
            ax_title.text(0.01, 0.75, f"Page {int(i)} — Rotated ROI & Intensity & CNN", fontsize=16, weight="bold")
            ax_title.text(0.01, 0.42, f"Generated at: {now}", fontsize=10)
            ax_title.text(
                0.01, 0.20,
                f"Line points: {page['line_pts']}\n"
                f"age_intensity(peaks)={page.get('age', None)}\n"
                f"age_cnn={page.get('cnn_age', None)}",
                fontsize=10, va="top"
            )

            ax1 = fig.add_subplot(gs[1:3, 0])
            ax1.set_title("Original (with line)")
            ax1.axis("off")
            ax1.imshow(page["orig_with_line"])

            ax2 = fig.add_subplot(gs[1:3, 1])
            ax2.set_title("ROI Preview (rotated crop)")
            ax2.axis("off")
            ax2.imshow(page["roi_preview"])

            ax3 = fig.add_subplot(gs[3:5, :])
            ax3.axis("off")
            ax3.set_title("Intensity (adaptive peaks)")
            ax3.imshow(page["profile_img"])

            ax4 = fig.add_subplot(gs[5, :])
            ax4.axis("off")
            ax4.text(0.01, 0.6, "Exported by Gradio tool.", fontsize=10)
        ...
        def export_pdf_all(pstore, n_imgs):
            if n_imgs is None or n_imgs == 0:
                return None, "<span class='accent'>⚠️</span> <span class='muted'>No images.</span>"
            ...
            with PdfPages(out_path) as pdf:
                for i in range(n_imgs):
                    ...
                    ax_title.axis("off")
                    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ax_title.text(0.01, 0.75, f"Page {i} — Rotated ROI & Intensity & CNN", fontsize=16, weight="bold")
                    ax_title.text(0.01, 0.42, f"Generated at: {now}", fontsize=10)
                    ax_title.text(
                        0.01, 0.20,
                    ...
                    ax1.set_title("Original (with line)")
                    ax1.axis("off")
                    if page["orig_with_line"] is not None:
                        ax1.imshow(page["orig_with_line"])
                    else:
                        ax1.text(0.5, 0.5, "No ROI yet", ha="center", va="center")

                    ax2 = fig.add_subplot(gs[1:3, 1])
                    ax2.set_title("ROI Preview")
                    ax2.axis("off")
                    if page["roi_preview"] is not None:
                        ax2.imshow(page["roi_preview"])
                    else:
                        ax2.text(0.5, 0.5, "No ROI", ha="center", va="center")

                    ax3 = fig.add_subplot(gs[3:5, :])
                    ax3.axis("off")
                    ax3.set_title("Intensity")
                    if page["profile_img"] is not None:
                        ax3.imshow(page["profile_img"])
                    else:
        ...
        def export_csv_all(pstore, paths):
            ...
            if not rows:
                return None, "<span class='accent'>⚠️</span> <span class='muted'>No pages with computed ages.</span>"
